...
cols = ["time", "1803_RX_LEVEL", "RX_DIFF_FROM_MAX"]
freq = 3
...
print(f"最大値: {df1['1803_RX_LEVEL'].max()}")
print(f"最小値: {df1['1803_RX_LEVEL'].min()}")
df1[cols[2]] = df1["1803_RX_LEVEL"].max() - df1["1803_RX_LEVEL"]
...
ax.set_xscale("log")
ax.set_xlabel("累積時間率(%)")
ax.set_ylabel("受信強度差分(dB)")
ax.set_title("受信強度累積時間分布 26GHz(最大値：-99.0dBからの差分))")
ax.xaxis.set_major_formatter(plt.FuncFormatter(lambda x, loc: "{:,}".format(float(x))))
plt.legend()
plt.savefig("rx_dist_26_max.png")
# plt.show()